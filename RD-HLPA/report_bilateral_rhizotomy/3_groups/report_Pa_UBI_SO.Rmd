---
title: "Bilateral rhizotomy. Analyse Pa for SO, UBI (L, R; Hand, Machine)"
...
rds_folder_name <- "../../rds/bilateral_rhizotomy/3_groups/pa/"
...
New Contrast between treatment as a table:

```{r hand_ubi_pa_table_new}
...
New Contrasts between treatment as a median +- 95% HPDCI:

```{r hand_ubi_pa_plot_new, echo=FALSE, fig.width = 8}

plt[["Hand UBI Pa by Trt3 NEW"]] <- emmc %>% emm_inorder %>% 
  ggplot(aes(y = fct_rev(interaction(Time, contrast, sep=", ", drop=TRUE)), x = .value, fill = contrast)) %>%
  my.stat_eyeh() +
  geom_vline(xintercept=0, linetype="dashed", color="black") +
  # coord_cartesian(ylim = c(-1, 1)) +
  labs(title=myname, y="", x="Pa, contrast: UBI vs SO")
print(plt[["Hand UBI Pa by Trt3 NEW"]])

```

Contrast of contrasts between treatment as a table:

```{r hand_ubi_pa_table_contrast_of_contrasts}
...
Contrast of contrasts as a median +- 95% HPDCI:

```{r hand_ubi_pa_contrast_of_contrasts, echo=FALSE, fig.width = 8}

plt[["Hand UBI Pa by Trt3 Contrast of contrasts"]] <- emmc %>% emm_inorder %>% 
  ggplot(aes(y = fct_rev(interaction(contrast1, contrast, sep=", ", drop=TRUE)), x = .value, fill = contrast)) %>%
  my.stat_eyeh() +
  geom_vline(xintercept=0, linetype="dashed", color="black") +
  # coord_cartesian(ylim = c(-1, 1)) +
  labs(title=myname, y="", x="Pa, contrast: UBI vs SO")
print(plt[["Hand UBI Pa by Trt3 Contrast of contrasts"]])

```

New Contrast between treatment as a table:

```{r hand_ubi_pa_table_new_reverse}
...
```{r hand_ubi_pa_plot_new_reverse, echo=FALSE, fig.width = 8}

plt[["Hand UBI Pa by Trt3 NEW reverse"]] <- emmc %>% emm_inorder %>% 
  ggplot(aes(y = fct_rev(interaction(Time, contrast, sep=", ", drop=TRUE)), x = .value, fill = contrast)) %>%
  my.stat_eyeh() +
  geom_vline(xintercept=0, linetype="dashed", color="black") +
  # coord_cartesian(ylim = c(-1, 1)) +
  labs(title=myname, y="", x="Pa, contrast: UBI vs SO")
print(plt[["Hand UBI Pa by Trt3 NEW reverse"]])

```

Contrast of contrasts between treatment as a table:

```{r hand_ubi_pa_table_contrast_of_contrasts1}
...
```{r hand_ubi_pa_contrast_of_contrasts1, echo=FALSE, fig.width = 8}

plt[["Hand UBI Pa by Trt3 Contrast of contrasts reverse"]] <- emmc %>% emm_inorder %>% 
  ggplot(aes(y = fct_rev(interaction(contrast1, contrast, sep=", ", drop=TRUE)), x = .value, fill = contrast)) %>%
  my.stat_eyeh() +
  geom_vline(xintercept=0, linetype="dashed", color="black") +
  # coord_cartesian(ylim = c(-1, 1)) +
  labs(title=myname, y="", x="Pa, contrast: UBI vs SO")
print(plt[["Hand UBI Pa by Trt3 Contrast of contrasts reverse"]])
...
```{r machine_ubi_pa_plot_new, echo=FALSE, fig.width = 8}

plt[["machine UBI Pa by Trt3 NEW"]] <- emmc %>% emm_inorder %>% 
  ggplot(aes(y = fct_rev(interaction(Time, contrast, sep=", ", drop=TRUE)), x = .value, fill = contrast)) %>%
  my.stat_eyeh() +
  geom_vline(xintercept=0, linetype="dashed", color="black") +
  # coord_cartesian(ylim = c(-1, 1)) +
  labs(title=myname, y="", x="Pa, contrast: UBI vs SO")
print(plt[["machine UBI Pa by Trt3 NEW"]])

```

Contrast of contrasts between treatment as a table:

```{r machine_ubi_pa_table_contrast_of_contrasts}
emmc <- contrast(emmc, simple="Time", method = "pairwise")
...
```{r machine_ubi_pa_contrast_of_contrasts, echo=FALSE, fig.width = 8}

plt[["machine UBI Pa by Trt3 Contrast of contrasts"]] <- emmc %>% emm_inorder %>% 
  ggplot(aes(y = fct_rev(interaction(contrast1, contrast, sep=", ", drop=TRUE)), x = .value, fill = contrast)) %>%
  my.stat_eyeh() +
  geom_vline(xintercept=0, linetype="dashed", color="black") +
  # coord_cartesian(ylim = c(-1, 1)) +
  labs(title=myname, y="", x="Pa, contrast: UBI vs SO")
print(plt[["machine UBI Pa by Trt3 Contrast of contrasts"]])

```

New Contrast between treatment as a table:

```{r machine_ubi_pa_table_new_reverse}
...
```{r machine_ubi_pa_plot_new_reverse, echo=FALSE, fig.width = 8}

plt[["machine UBI Pa by Trt3 NEW reverse"]] <- emmc %>% emm_inorder %>% 
  ggplot(aes(y = fct_rev(interaction(Time, contrast, sep=", ", drop=TRUE)), x = .value, fill = contrast)) %>%
  my.stat_eyeh() +
  geom_vline(xintercept=0, linetype="dashed", color="black") +
  # coord_cartesian(ylim = c(-1, 1)) +
  labs(title=myname, y="", x="Pa, contrast: UBI vs SO")
print(plt[["machine UBI Pa by Trt3 NEW reverse"]])
...
```{r machine_ubi_pa_table_contrast_of_contrasts1}
...
```{r machine_ubi_pa_contrast_of_contrasts1, echo=FALSE, fig.width = 8}

plt[["machine UBI Pa by Trt3 Contrast of contrasts reverse"]] <- emmc %>% emm_inorder %>% 
  ggplot(aes(y = fct_rev(interaction(contrast1, contrast, sep=", ", drop=TRUE)), x = .value, fill = contrast)) %>%
  my.stat_eyeh() +
  geom_vline(xintercept=0, linetype="dashed", color="black") +
  # coord_cartesian(ylim = c(-1, 1)) +
  labs(title=myname, y="", x="Pa, contrast: UBI vs SO")
print(plt[["machine UBI Pa by Trt3 Contrast of contrasts reverse"]])
