---
title: "Bilateral rhizotomy. Analyse pContra UBI"
...
rds_folder_name <- "../../rds/bilateral_rhizotomy/pcontra/"
...
co_d <- data %>% 
  ...
  flextable(col_keys = c("Trt3", "Measuerment method", "3h0m", "3h15m")) %>% autofit
...
m.lat <- brm(
  ...
  nContra | trials(nTrials) ~ 0 + Trt2:Time,
...
emm <- emmeans(m.lat, ~ Trt2|Time, transform = "response", nesting = NULL)
emm_show(emm)
...
plt[["hand_p_Contra emm"]] <- emm %>% emm_inorder %>%
  ggplot(aes(y = fct_rev(interaction(Trt2, Time, sep=", ", drop=TRUE)), x = .value, fill = Trt2)) %>%
  my.stat_eyeh() +
