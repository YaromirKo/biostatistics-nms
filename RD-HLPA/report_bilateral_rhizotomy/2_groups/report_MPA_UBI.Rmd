---
title: "Bilateral rhizotomy. Analyse MPA"
...
rds_folder_name <- "../../rds/bilateral_rhizotomy/mpa/"
...
plt[["UBI MPA by Side (hand)"]] <- emmc %>% emm_inorder %>% filter(Side == ".") %>%
  ggplot(aes( y = fct_rev(interaction(Time, contrast, sep=", ", drop=TRUE)), x=.value, fill=contrast)) %>%
  my.stat_eyeh() +
  geom_vline(xintercept=0, linetype="dashed", color="black") +
  labs(title=myname, y="", x="MPA in mm, contrast: L-UBI vs R-UBI")

print(plt[["UBI MPA by Side (hand)"]])
...
plt[["UBI MPA by Side (machine)"]] <- emmc %>% emm_inorder %>% filter(Side == ".") %>%
  ggplot(aes( y = fct_rev(interaction(Time, contrast, sep=", ", drop=TRUE)), x=.value, fill=contrast)) %>%
  my.stat_eyeh() +
  geom_vline(xintercept=0, linetype="dashed", color="black") +
  labs(title=myname, y="", x="MPA in mm, contrast: L-UBI vs R-UBI")

print(plt[["UBI MPA by Side (machine)"]])
