...
rds_folder_name <- "../rds/sf/V/"

load("SF 0-1 0.4-1 1-2 0-2 0.4-2 loess(symmetric span0.4)_Variances.RData")
# rm(sf) # conserve memory, we do not need the raw data

dta <- sf_CI_V_I %>%
  filter(dT %in% c("T_0_2")) %>%
  pivot_longer(c(V_i, V_c), names_to = "LaterV", values_to = "V") %>% 
  mutate(LaterV = factor(LaterV, c("V_c", "V_i"))) %>% 
  droplevels(.)

```

The median (bars) and 95% HPDCI (whiskers) for SF V data points. __NOT__ the uncertainties estimated by Bayesian model.
...
plt[["all_group"]] <- dta %>% 
  group_by(Trt, dT, LaterV) %>% 
  median_hdci(V, .width = 0.95) %>%
  ungroup() %>%
  ggplot(aes(x=interaction(dT, LaterV, sep=", ", drop=TRUE), y=V, fill=Trt)) + 
    ...
    labs(title=myname, x="Medians and 95% HDCI", y="V")
...
plt[["preSO vs SO vs SO+Rhiz"]] <- 
  dta_SO %>% 
  group_by(Op3, dT, LaterV) %>% 
  median_hdci(V, .width = 0.95) %>%
  ungroup() %>%
  ggplot(aes(x=interaction(dT, LaterV, sep=", ", drop=TRUE), y=V, fill=Op3)) + 
    geom_bar(position=position_dodge(), stat="identity") +
    geom_errorbar(aes(ymin=.lower, ymax=.upper),
                  width=.2,   # Width of the error bars
                  position=position_dodge(.9)) +
...
## SF V: Bayesian regression, student family
...
get_prior(data = dta_SO,
  family = student,
  V ~ 0 + Op3:LaterV + weight + repN + (1|RatID))
...
std.prior <- brm(
  ...
  family = student,
  V ~ 0 + Op3:LaterV + weight + repN + (1|RatID),
  # V ~ 0 + Op3 + (1|RatID), # eliminate non-significant confounders
    # V ~ 0 + Op3:dT + weight + repN + (1|RatID),
  # V ~ 0 + Op3:dT + (1|RatID), # eliminate non-significant confounders
...
cond_eff <- conditional_effects(std.prior,
                                "Op3",
                                method="fitted",
                                conditions=make_conditions(dta_SO, vars = c("LaterV"))
                                )
...
## SF V: MCMC conversion diagnostics
...
plt[["SF preSO vs SO vs SO+Rhiz emm"]] <- emm %>% emm_inorder %>%
  ggplot(aes(x = interaction(Op3, LaterV, sep=", ", drop=TRUE), y = .value, fill = Op3)) %>%
  my.stat_eye() +
  geom_hline(yintercept=0, linetype="dashed", color="black") +
  labs(title=myname, x="Estimated Marginal Medians and 95% HPDCI", y="V")
...
<!--       data = { -->
<!--         dta_SO %>% -->
<!--           group_by(RatID, Op3, LaterV) %>% -->
<!--           summarise(.value = mean(V), .groups = "drop") -->
<!--       }, -->
<!--       aes(y=.value)) -->
...
emmc <- rbind(contrast(emm, simple = "Op3", method = "revpairwise"),
              contrast(emm, simple = "LaterV", method = "consec"))
emm_show(emmc)
...
plt[["SF by preSO vs SO vs SO+Rhiz"]] <- emmc %>% gather_emmeans_draws %>% filter(LaterV == ".") %>% 
  ggplot(aes(x = interaction(Op3, contrast, sep=", ", drop=TRUE), y = .value, fill = Op3)) %>%
  my.stat_eye() +
  geom_hline(yintercept=0, linetype="dashed", color="black") +
  labs(title=myname, x="0-2 sec", y="V, between operations")
...
myname <- "SF 0-2 span=0.4 [SO vs UBI]"
...
cond_eff <- conditional_effects(std.prior,
                                "Op3",
                                method="fitted",
                                conditions=make_conditions(dta_SO_UBI, vars = c("LaterV"))
                                )
...
emm <- emmeans(std.fit, ~ Op3*LaterV, transform = "response")
emm_show(emm)
...
Contrasts SO vs UBI.L vs UBI.R as a table:
...
plt[["SF by SO vs UBI-L vs UBI-R"]] <- emmc %>% gather_emmeans_draws %>% filter(LaterV == ".") %>% 
  ggplot(aes(x = interaction(Op3, contrast, sep=", ", drop=TRUE), y = .value, fill = Op3)) %>%
  my.stat_eye() +
  geom_hline(yintercept=0, linetype="dashed", color="black") +
  labs(title=myname, x="Sp, 0-2 sec", y="V, between operations")
...
```{r input2_so_ubi_rhiz, echo=FALSE}
...
```{r}
notify(msg="SO+Rhiz L and R UBI+ Rhiz")
```
...
Contrasts SO+Rhiz vs UBI+Rhiz.L vs UBI+Rhiz.R as a table:
...
plt[["SF by SO+Rhiz vs UBI+Rhiz-L vs UBI+Rhiz-R"]] <- emmc %>% gather_emmeans_draws %>% filter(LaterV == ".") %>% 
  ggplot(aes(x = interaction(Op3, contrast, sep=", ", drop=TRUE), y = .value, fill = Op3)) %>%
  my.stat_eye() +
  geom_hline(yintercept=0, linetype="dashed", color="black") +
  labs(title=myname, x="0-2 sec", y="V, between operations")
...
cond_eff <- conditional_effects(std.prior,
                                "Op3",
                                method="fitted",
                                conditions=make_conditions(dta_ADX, vars = c("LaterV"))
                                )
...
plt[["SF by preADX vs ADX-L vs ADX-R"]] <- emmc %>% gather_emmeans_draws %>% filter(LaterV == ".") %>% 
...
plt[["SF ADX vs ADX+UBI-L vs ADX+UBI-R pp_check (hand)"]] <- pp_check(std.fit, ndraws = 11)

print(plt[["SF ADX vs ADX+UBI-L vs ADX+UBI-R pp_check (hand)"]])
...
plt[["SF ADX vs ADX+UBI-L vs ADX+UBI-R ac (hand)"]] <- stan_ac(std.fit$fit)

print(plt[["SF ADX vs ADX+UBI-L vs ADX+UBI-R ac (hand)"]])
...
plt[["SO vs UBI vs ADX+UBI (L + R)"]] <- 
  ...
  median_hdci(V, .width = 0.95) %>%
  ungroup() %>%
  ggplot(aes(x=interaction(dT, sep=", ", drop=TRUE), y=V, fill=Op3)) + 
    geom_bar(position=position_dodge(), stat="identity") +
    geom_errorbar(aes(ymin=.lower, ymax=.upper),
                  width=.2,   # Width of the error bars
                  position=position_dodge(.9)) +
...
plt[["preSO vs SO vs preUBI-[L,R] vs UBI-[L,R] vs UBI+Rhiz-[L,R]"]] <- 
  ...
  median_hdci(V, .width = 0.95) %>%
  ungroup() %>%
  ggplot(aes(x=interaction(Time, dT, sep=", ", drop=TRUE), y=V, fill=Op3)) + 
    geom_bar(position=position_dodge(), stat="identity") +
    geom_errorbar(aes(ymin=.lower, ymax=.upper),
                  width=.2,   # Width of the error bars
                  position=position_dodge(.9)) +
...
get_prior(data = dta_SO_UBI_Rhiz,
  family = student,
  V ~ 0 + Op3:Time + weight + repN + (1|RatID))
...
std.prior <- brm(
  ...
  iter = 4e4,
  sample_prior = "only",
  file = paste0(rds_folder_name, "pre_so_ubi_rhiz_check_prior.rds"),
  file_refit = "on_change"
)
...
std.fit <-  update(std.prior, 
                   ...
                   seed = my.seed, 
                   cores = my.cores,
                   file = paste0(rds_folder_name, "pre_so_ubi_rhiz_model_summary_max_treedepth=15.rds"),
                   file_refit = "on_change")
summary(std.fit)
...
std.prior <- brm(
  ...
  iter = 4e4,
  sample_prior = "only",
  file = paste0(rds_folder_name, "pre_adx_ubi_lr_check_prior.rds"),
  file_refit = "on_change"
)
...
std.fit <-  update(std.prior, 
                   ...
                   seed = my.seed, 
                   cores = my.cores,
                   file = paste0(rds_folder_name, "pre_adx_ubi_lr_model_summary_max_treedepth=15.rds"),
                   file_refit = "on_change")
summary(std.fit)
