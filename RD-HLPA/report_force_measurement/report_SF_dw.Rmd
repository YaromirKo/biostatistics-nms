...
plt[["all_group"]] <- dta %>% 
  ...
  ungroup() %>%
  ggplot(aes(x=interaction(dT, sep=", ", drop=TRUE), y=W, fill=Trt)) + 
...
std.prior <- brm(
  ...
  family = student,
  W ~ 0 + Op3 + weight + repN + (1|RatID),
  # W ~ 0 + Op3 + (1|RatID), # eliminate non-significant confounders
    # W ~ 0 + Op3:dT + weight + repN + (1|RatID),
  # W ~ 0 + Op3:dT + (1|RatID), # eliminate non-significant confounders
...
plt[["SF by preSO vs SO vs SO+Rhiz"]] <- emmc %>% gather_emmeans_draws %>%
  ...
  my.stat_eye() +
  geom_hline(yintercept=0, linetype="dashed", color="black") +
  labs(title=myname, x="0-2 sec", y="dW, between operations")
...
plt[["SF by SO vs UBI-L vs UBI-R"]] <- emmc %>% gather_emmeans_draws %>%
  ...
  my.stat_eye() +
  geom_hline(yintercept=0, linetype="dashed", color="black") +
  labs(title=myname, x="Sp, 0-2 sec", y="dW, between operations")

print(plt[["SF by SO vs UBI-L vs UBI-R"]])

```

Contrasts reverse SO vs UBI.L vs UBI.R as a table:
...
Contrasts addition SO vs UBI.L, UBI.R as a table:

```{r sf_contrasts_so_ubi_reverse_addition}
...
Contrasts addition SO vs UBI.L, UBI.R as a median +- 95% HPDCI (HPDCI = highest posterior density continuous interval):

```{r sf_by_so_ubi_reverse_addition, echo=FALSE, fig.width = 8}

plt[["SF by SO vs UBI-L vs UBI-R addition"]] <- emmc %>% gather_emmeans_draws %>%
  ggplot(aes(x = interaction(contrast, sep=", ", drop=TRUE), y = .value, fill = contrast)) %>%
  my.stat_eye() +
  geom_hline(yintercept=0, linetype="dashed", color="black") +
  labs(title=myname, x="Sp, 0-2 sec", y="dW, between operations")

print(plt[["SF by SO vs UBI-L vs UBI-R addition"]])
...
Contrasts reverse SO+Rhiz vs UBI+Rhiz.L vs UBI+Rhiz.R as a table:
...
Contrasts SO+Rhiz vs UBI+Rhiz.L, UBI+Rhiz.R as a table:
...
plt[["SF addition by SO+Rhiz vs UBI+Rhiz-L vs UBI+Rhiz-R"]] <- emmc %>% gather_emmeans_draws %>%
  ggplot(aes(x = interaction(contrast, sep=", ", drop=TRUE), y = .value, fill = contrast)) %>%
  my.stat_eye() +
  geom_hline(yintercept=0, linetype="dashed", color="black") +
  labs(title=myname, x="0-2 sec", y="dW, between operations")

print(plt[["SF addition by SO+Rhiz vs UBI+Rhiz-L vs UBI+Rhiz-R"]])
...
myname <- "SF 0-2 span=0.4 [UBI-[L,R] vs UBI+Rhiz-[L,R] addition]"
...
plt[["UBI-[L,R] vs UBI+Rhiz-[L,R] addition"]] <- 
  ...
  median_hdci(W, .width = 0.95) %>%
  ungroup() %>%
  ggplot(aes(x=interaction(Time, dT, sep=", ", drop=TRUE), y=W, fill=Op2)) + 
    geom_bar(position=position_dodge(), stat="identity") +
    geom_errorbar(aes(ymin=.lower, ymax=.upper),
                  width=.2,   # Width of the error bars
                  position=position_dodge(.9)) +
...
get_prior(data = dta_UBI_Rhiz,
  family = student,
  W ~ 0 + Op2:Time + weight + repN + (1|RatID))
...
std.prior <- brm(
  ...
  iter = 4e4,
  sample_prior = "only",
  file = paste0(rds_folder_name, "addition_v1_ubi_rhiz_check_prior.rds"),
  file_refit = "on_change"
)
...
std.fit <-  update(std.prior, 
                   ...
                   seed = my.seed, 
                   cores = my.cores,
                   file = paste0(rds_folder_name, "addition_v1_ubi_rhiz_model_summary_max_treedepth=15.rds"),
                   file_refit = "on_change")
summary(std.fit)
...
```{r sf_by_time_ubi_lr_rhiz_addition, echo=FALSE, fig.width = 8}

plt[["SF addition by Time UBI-[L,R] vs UBI+Rhiz-[L,R]"]] <- emmc %>% gather_emmeans_draws %>% 
  ggplot(aes(x = interaction(Op2, contrast, sep=", ", drop=TRUE), y = .value, fill = Op2)) %>%
  my.stat_eye() +
  geom_hline(yintercept=0, linetype="dashed", color="black") +
  labs(title=myname, x="Sp, 0-2 sec", y="dW, between operations")

print(plt[["SF addition by Time UBI-[L,R] vs UBI+Rhiz-[L,R]"]])

```

Contrasts reverse UBI-[L,R] vs UBI+Rhiz-[L,R] as a table:

```{r sf_contrasts_ubi_lr_rhiz_addition_reverse}
...
```{r sf_by_time_ubi_lr_rhiz_addition_reverse, echo=FALSE, fig.width = 8}

plt[["SF reverse addition by Time UBI-[L,R] vs UBI+Rhiz-[L,R]"]] <- emmc %>% gather_emmeans_draws %>% 
  ggplot(aes(x = interaction(Op2, contrast, sep=", ", drop=TRUE), y = .value, fill = Op2)) %>%
  my.stat_eye() +
  geom_hline(yintercept=0, linetype="dashed", color="black") +
  labs(title=myname, x="Sp, 0-2 sec", y="dW, between operations")

print(plt[["SF reverse addition by Time UBI-[L,R] vs UBI+Rhiz-[L,R]"]])

```

## SF $\Delta W$: Contrasts of contrasts
...
Contrasts reverse ADX vs ADX+UBI-L vs ADX+UBI-R as a median +- 95% HPDCI (HPDCI = highest posterior density continuous interval):
...
Contrasts reverse SO vs UBI vs ADX+UBI (L + R) as a median +- 95% HPDCI (HPDCI = highest posterior density continuous interval):
...
plt[["SO vs UBI vs ADX+UBI (L + R) 0min and 180min"]] <- 
  ...
  median_hdci(W, .width = 0.95) %>%
  ungroup() %>%
  ggplot(aes(x=interaction(dT, Time, sep=", ", drop=TRUE), y=W, fill=Op3)) + 
    geom_bar(position=position_dodge(), stat="identity") +
    geom_errorbar(aes(ymin=.lower, ymax=.upper),
                  width=.2,   # Width of the error bars
                  position=position_dodge(.9)) +
...
get_prior(data = dta_ADX,
  family = student,
  W ~ 0 + Op3:Time + weight + repN + (1|RatID))
...
<!--   median_hdci(W, .width = 0.95) %>% -->
<!--   ungroup() %>% -->
<!--   ggplot(aes(x=interaction(Time, dT, sep=", ", drop=TRUE), y=W, fill=Op3)) +  -->
<!--     geom_bar(position=position_dodge(), stat="identity") + -->
<!--     geom_errorbar(aes(ymin=.lower, ymax=.upper), -->
<!--                   width=.2,   # Width of the error bars -->
<!--                   position=position_dodge(.9)) + -->
