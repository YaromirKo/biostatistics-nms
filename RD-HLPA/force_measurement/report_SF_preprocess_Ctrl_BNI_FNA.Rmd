---
title: 'SF: Read and preprocess raw stretch force measurements Ctrl, BNI, FNA'
...
PATH_DIR_ROOT <- "../data"
PATH_FOLDER_FORCE_MEASUREMENT <- "../data/SF/" 
PATH_SF_DATA_CSV <- paste0(PATH_FOLDER_FORCE_MEASUREMENT, "sf_ctrl_bni_fna_source_data_20230506.csv")

FILE_NAME = '-ForceValues.xlsx'
PATTERN = paste0('*', FILE_NAME)

DESCRIPTION_FILE_NAME <- "SDU-RDPA-Stat_v2.xlsx"
SHEET <- "Data" # for description
...
my.rwData <- function() {
  ...
  sf <-
    ...
    # mutate(
    #   SF = SF*(-1),
    #   Trt = ifelse(replication %in% c(1:6), ifelse(Group3 == 'Ctrl', '0h.Ctrl', paste0('0h.', Group3)),
    #                ifelse(Group3 == 'Ctrl', '3h.Ctrl', paste0('3h.', Group3))),
    #   Operation.side = recode_factor(Operation.side, Left='L', Right='R'),
    #   Group3 = ifelse(Group3 == 'Ctrl', 'Ctrl', paste0(Operation.side, '.', Group3)),
    #   Group3 = factor(Group3, c("Ctrl", "L.BNI", "R.BNI", "L.FNA", "R.FNA")),
    #   Period = ifelse(replication %in% c(1:6), "0h", "3h"),
    #   Period = factor(Period, c("0h", "3h"))
    # )
...
d.sum <- sf %>% 
  arrange(Group3, Period) %>% 
  group_by(Group3, Period) %>% 
  summarise(rats = length(unique(RatID))) %>%
  pivot_wider(names_from=c(Group3), values_from = rats)
...
name_fig <- "SDU-115, 3h.BNI.L, replicate 12"
...
save(sf_CI_dW, sf_CI_AI, sf_LR_dW, sf_LR_AI, sf, file=paste0(myname, "_Ctrl_BNI_FNA_0_3_h_rats_v2.RData"))
