---
title: "SF, LR AI (Ctrl, BNI, FNA): Bayesian regression of smoothed stretch forces"
...
load("SF Sp 0-1 0.4-1 1-2 0-2 0.4-2 loess(symmetric span0.3)_Ctrl_BNI_FNA_0_3_h_rats.RData")
# rm(sf) # conserve memory, we do not need the raw data

getTimeByStr <- Vectorize(function(date, pattern=".") {
  time <- str_split(date, pattern)[[1]]
  print(str_split(date, pattern))
  return(time)
})
...
myname <- paste0("SF 0-2 span=", my.span, " median and 95% HPDCI all group")

# adapted from http://www.cookbook-r.com/Graphs/Plotting_means_and_error_bars_(ggplot2)/
plt[["Ctr_BNI_FNA_groups"]] <- dta %>% 
