...
std.prior <- brm(
  ...
  iter = 4e4,
  sample_prior = "only",
  file = paste0(rds_folder_name, "ctrl_nti_nal_check_prior_20230407.rds"),
...
std.fit <- update(std.prior, 
                   ...
                   seed = my.seed, 
                   cores = my.cores,
                   file = paste0(rds_folder_name, "ctrl_nti_nal_model_summary_20230407.rds"),
                   file_refit = "on_change")
summary(std.fit)
...
plt[["SF Contrast of contrasts NTI, Naloxone vs Ctrl"]] <- emmc_2 %>% emm_inorder %>% 
  ...
  ggplot(aes(y = .value, x = interaction(contrast, sep=", ", drop=TRUE), fill = contrast)) %>%
  my.stat_eye() +
  geom_hline(yintercept=0, linetype="dashed", color="black") +
