...
std.prior <- brm(
  ...
  iter = 4e4,
  sample_prior = "only",
  file = paste0(rds_folder_name, "ctrl_bni_fna_0h_check_prior_20230407_v2.rds"),
...
std.fit <-  update(std.prior, 
                   ...
                   seed = my.seed, 
                   cores = my.cores,
                   file = paste0(rds_folder_name, "ctrl_bni_fna_0h_model_summary_20230407_v2.rds"),
                   file_refit = "on_change")
summary(std.fit)
...
std.prior <- brm(
  ...
  iter = 4e4,
  sample_prior = "only",
  file = paste0(rds_folder_name, "ctrl_bni_fna_3h_check_prior_20230407_v2.rds"),
  file_refit = "on_change"
)
...
std.fit <-  update(std.prior, 
                   ...
                   seed = my.seed, 
                   cores = my.cores,
                   file = paste0(rds_folder_name, "ctrl_bni_fna_3h_model_summary_20230407_v2.rds"),
                   file_refit = "on_change")
summary(std.fit)
...
std.prior <- brm(
  ...
  iter = 4e4,
  sample_prior = "only",
  file = paste0(rds_folder_name, "ctrl_bni_fna_0h_3h_check_prior_20230407_v2.rds"),
  file_refit = "on_change"
)
...
std.fit <-  update(std.prior, 
                   ...
                   seed = my.seed, 
                   cores = my.cores,
                   file = paste0(rds_folder_name, "ctrl_bni_fna_0h_3h_model_summary_20230407_v2.rds"),
                   file_refit = "on_change")
summary(std.fit)
...
plt[["SF Contrast of contrasts BNI, FNA, Ctrl 3h"]] <- emmc_2 %>% emm_inorder %>% 
  ...
  ggplot(aes(y = .value, x = interaction(contrast, Time, sep=", ", drop=TRUE), fill = contrast)) %>%
  my.stat_eye() +
  geom_hline(yintercept=0, linetype="dashed", color="black") +
  labs(title=myname, x="dW", y="")
