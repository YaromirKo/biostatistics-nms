...
path_rds <- "rds/vertical_analysis/mpa1/" # folder to store the results 
...
std.prior <- brm(
  ...
  iter = 4e4,
  sample_prior = "only",
  file = paste0(full_rds_path, "ctrl_check_prior.v4.rds"),
...
std.fit <-  update(std.prior, 
                   ...
                   seed = my.seed, 
                   cores = my.cores,
                   file = paste0(full_rds_path, "ctrl_model_summary.v2.rds"),
                   file_refit = "on_change")
summary(std.fit)
...
std.prior <- brm(
  ...
  iter = 4e4,
  sample_prior = "only",
  file = paste0(full_rds_path, "ctrl_check_prior.rds"),
  file_refit = "on_change"
)
...
std.fit <-  update(std.prior, 
                   ...
                   seed = my.seed, 
                   cores = my.cores,
                   file = paste0(full_rds_path, "ctrl_model_summary.rds"),
                   file_refit = "on_change")
summary(std.fit)
...
plt[["Ctrl v2 MPA pp_check (hand)"]] <- pp_check(std.fit, ndraws = 21)

print(plt[["Ctrl v2 MPA pp_check (hand)"]])
...
plt[["Ctrl v2 MPA ac (hand)"]] <- stan_ac(std.fit$fit)

print(plt[["Ctrl v2 MPA ac (hand)"]])
...
plt[["Ctrl v2 MPA emm (hand)"]] <- emm %>% emm_inorder %>%
...
print(plt[["Ctrl v2 MPA emm (hand)"]])
...
plt[["Ctrl v2 MPA emm (hand) with Data Points"]] <- 
  plt[["Ctrl v2 MPA emm (hand)"]] +
    geom_point(
      pch = 4,
      position = position_nudge(y=0.15),
      data = {
...
print(plt[["Ctrl v2 MPA emm (hand) with Data Points"]])
...
plt[["Ctrl v2 MPA by Side (hand)"]] <- emmc %>% emm_inorder %>%
  ggplot(aes( y = fct_rev(interaction(contrast, sep=", ", drop=TRUE)), x=.value, fill=contrast)) %>%
  my.stat_eyeh() +
  geom_vline(xintercept=0, linetype="dashed", color="black") +
  labs(title=myname, y="", x="MPA in mm, contrast: Ctrl vs L+R-Saline")

print(plt[["Ctrl v2 MPA by Side (hand)"]])
...
std.fit <-  update(std.prior, 
                   ...
                   seed = my.seed, 
                   cores = my.cores,
                   file = paste0(full_rds_path, "saline_model_summary.ad999.maxtr15.rds"),
                   file_refit = "on_change")
summary(std.fit)
...
std.prior <- brm(
  ...
  iter = 4e4,
  sample_prior = "only",
  file = paste0(full_rds_path, "saline_time_check_prior.rds"),
  file_refit = "on_change"
)
...
plt[["Saline v2 MPA prior vs data (hand)"]] <- plot(cond_eff, 
                                       points = TRUE, 
                                       point_args = list(width = 0.1, shape="o", col="red"),
                                       plot=FALSE)[[1]] + 
                                  geom_hline(yintercept=0, linetype="dashed", color="black")
...
set.seed(2)
plt[["Saline v2 MPA prior pp_check (hand)"]] <- pp_check(std.prior, ndraws = 11)

print(cond_eff[[1]])
print(plt[["Saline v2 MPA prior vs data (hand)"]])
print(plt[["Saline v2 MPA prior pp_check (hand)"]])
...
std.fit <-  update(std.prior, 
                   ...
                   seed = my.seed, 
                   cores = my.cores,
                   file = paste0(full_rds_path, "saline_time_model_summary.ad999.maxtr15.rds"),
                   file_refit = "on_change")
summary(std.fit)
...
plt[["Saline v2 MPA pp_check (hand)"]] <- pp_check(std.fit, ndraws = 21)

print(plt[["Saline v2 MPA pp_check (hand)"]])
...
plt[["Saline v2 MPA ac (hand)"]] <- stan_ac(std.fit$fit)

print(plt[["Saline v2 MPA ac (hand)"]])
...
plt[["Saline v2 MPA emm (hand)"]] <- emm %>% emm_inorder %>%
...
print(plt[["Saline v2 MPA emm (hand)"]])
...
plt[["Saline v2 MPA emm (hand) with Data Points"]] <- 
  plt[["Saline v2 MPA emm (hand)"]] +
    geom_point(
      pch = 4,
      position = position_nudge(y=0.15),
      data = {
...
print(plt[["Saline v2 MPA emm (hand) with Data Points"]])
...
plt[["Saline v2 MPA by Side (hand)"]] <- emmc %>% emm_inorder %>%
  ggplot(aes( y = fct_rev(interaction(contrast, sep=", ", drop=TRUE)), x=.value, fill=contrast)) %>%
  my.stat_eyeh() +
  geom_vline(xintercept=0, linetype="dashed", color="black") +
  labs(title=myname, y="", x="MPA in mm, contrast: Saline 3h vs 4h")

print(plt[["Saline v2 MPA by Side (hand)"]])
...
std.prior <- brm(
  ...
  iter = 4e4,
  sample_prior = "only",
  file = paste0(full_rds_path, "machine_ctrl_check_prior.gaussian3010normal03.rds"),
  file_refit = "on_change"
)
...
std.fit <-  update(std.prior, 
                   ...
                   seed = my.seed, 
                   cores = my.cores,
                   file = paste0(full_rds_path, "machine_ctrl_model_summary.gaussian3010normal03.rds"),
                   file_refit = "on_change")
summary(std.fit)
...
std.fit <-  update(std.prior, 
                   ...
                   seed = my.seed, 
                   cores = my.cores,
                   file = paste0(full_rds_path, "machine_saline_model_summary.ad999.maxtr15.rds"),
                   file_refit = "on_change")
summary(std.fit)
